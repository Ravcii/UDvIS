...
Если всё сделано правильно, то появится следующая магия:
...
Последнее и самое интересное - Трансформация данные.
...
Нормализацию в виде таблиц поддерживают не все выходные источники (т.к. Airbyte работает не только с базами, но и REST-API, файлами и многим другим), но у вас должна эта опция быть. Если интересно как точно работает - [тут (англ)](https://docs.airbyte.com/understanding-airbyte/basic-normalization/)

Если мы хотим добавить свою, самостоятельную нормализацию \ трансформацию, то нужно указать _Custom Transformations_.
...
- Entrypoint arguments for dbt cli to run the project - Как запускать dbt. Помните вы делали переменные вместо логина и пароля? Вот тут теперь мы можем их указать и они у нас будут локально только в этой системе, а не где-то на гите для всеобщего обозрения.
...
Если у вас где-то есть ошибки, то ловить их нужно тут.
